...
def draw_headpose_det(det, input_img,sess,sess2):
        ...
        face_roi = cv2.resize(img_cut, (64, 64))
        ...
        draw_axis(img_raw, yaw, pitch, roll, tdx=(x2 - x1) // 2 + x1, tdy=(y2 - y1) // 2 + y1, size=50)




        yaw_txt = 'yaw:%.4f' % yaw
        roll_txt = 'roll:%.4f' % roll
        pitch_txt = 'pitch:%.4f' % pitch
...
if __name__ == '__main__':
    ...
    # 初始化头部姿态估计网络
    sess = onnxruntime.InferenceSession('./weights/fsanet_pre-trained/fsanet-1x1-iter-688590.onnx')
    sess2 = onnxruntime.InferenceSession('./weights/fsanet_pre-trained/fsanet-var-iter-688590.onnx')
    ...
    while True:
        ...
        for i in range(1):  # range(100)
            ...
            for num, b in enumerate(dets):  # dets中包含了人脸框和五个特征点的坐标
                ...
                draw_headpose_det(b, img_raw, sess, sess2)
                ...
                cx = b[0]
                cy = b[1] + 12
                cv2.putText(img_raw, confidence_text + "_" + str(num), (cx, cy),
            ...
            for i in range(detections.size(1)):
                ...
                while detections[0, i, j, 0] >= 0.4:
                    ...
                    if label_name == 'open_mouth':
                        flag_Y = True
                    object_txt = '%s:%.2f' % (label_name, score)
                    ...
                    cv2.putText(img_raw, object_txt, (int(pt[0]), int(pt[1]) + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.4,
                                (255, 255, 255),
                                1, 8)
                    j += 1
                    num_rec += 1
